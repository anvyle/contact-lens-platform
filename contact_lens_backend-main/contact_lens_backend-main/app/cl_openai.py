...
class OpenAIFineTuner:
    ...
    def wait_for_file_processing(self, sleep_time=20):
        while self.file_object.status != 'processed':
            ...
            logging.info("File Status: ", self.file_object.status)
    ...
    def wait_for_fine_tuning(self, sleep_time=45):
        while self.fine_tuning_job.status != 'succeeded':
            ...
            logging.info("Job Status: ", self.fine_tuning_job.status)
...
class Embedder:
    ...
    def embed_pdf_chunks_to_chroma(self, pdf_file, collection_name):
        ...
        logging.info(f"Embeddings from {pdf_file} saved to Chroma collection: {collection_name}")

        destination_folder = "./public/pdfs"
        os.makedirs(destination_folder, exist_ok=True)
        shutil.move(pdf_file, os.path.join(destination_folder, os.path.basename(pdf_file)))
        logging.info(f"Moved {pdf_file} to {destination_folder}")
    ...
    def scrape_website_content(self, url):
        ...
        if response.status_code == 200:
            ...
            logging.info(f"Failed to retrieve content from {url}: {response.status_code}")
            return
    ...
    def embed_website_to_chroma(self, url, collection_name):
        ...
        if text:
            ...
            logging.info(f"Embeddings from {url} saved to Chroma collection: {collection_name}")
            return True
        else:
            return False
