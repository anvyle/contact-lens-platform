...
async def google_callback(code: str, session: SessionDep): # type: ignore
    try:
...
def register_user(session: SessionDep, user_in: UserRegister): # type: ignore # type: ignore
...
def login_access_token(session: SessionDep, user_in: UserLogin): # type: ignore # type: ignore
...
@router.post("/me")
def update_user_me(*, session: SessionDep, user_in: UserUpdateMe, current_user: CurrentUser): # type: ignore
    if user_in.email:
